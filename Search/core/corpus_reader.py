...
django.setup()
from Search.core.mm_seg import fmm_seg, dictionary
...
def read_documents_from_file():
    ...
    res=lastNews.objects.filter(searchlization ='N').all()
    # print(len(res))
    for item in res:
        id=item.hashid
        content=item.title+item.content
        document_texts.append([id,content])
...
if __name__ == '__main__':
    res=read_documents_from_file()
    for item in res:
        res=fmm_seg(item[1],dictionary)
        print(res)
    '''
    测试
    '''
